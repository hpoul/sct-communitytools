...
class IncludeMacro(mdx_macros.PreprocessorMacro):
    def handlePreprocessorMacroCall(self, params):
        if 'url' in params:
            cache_key = settings.CACHE_MIDDLEWARE_KEY_PREFIX + 'sph_community_includemacro_' + params[
                'url'] + '_' + params.get('start', '') + '_' + params.get('end', '');
...
class NewsRSSLinkMacro(object):
    ...
    def handleMacroCall(self, doc, params):
        ...
        return HTML(
            '<a href="%(url)s"><img src="%(media_url)ssphene/community/icons/feed-icon-14x14.png" border="0" alt="RSS Feed of latest threads" title="RSS Feed of latest threads" /></a>' % {
                'url': url, 'media_url': settings.STATIC_URL})
