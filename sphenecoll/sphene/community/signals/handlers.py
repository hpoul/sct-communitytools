...
from .signals import profile_edit_init_form, profile_edit_save_form, profile_display
...
def community_advprofile_edit_init_form(sender, instance, signal, *args, **kwargs):
    ...
    if profile.avatar:
        instance.fields['community_advprofile_avatar_remove'] = djangoforms.BooleanField(label=_(u'Delete avatar'),
                                                                                         required=False)

    max_width = get_sph_setting('community_avatar_max_width')
    max_height = get_sph_setting('community_avatar_max_height')
    instance.fields['community_advprofile_avatar'] = djangoforms.ImageField(label=_(u'Avatar'), help_text=_(
        u'Avatar with maximum size of %(max_width)sx%(max_height)s' % {'max_width': max_width,
                                                                       'max_height': max_height}), required=False, )
...
def community_advprofile_edit_save_form(sender, instance, signal, request, **kwargs):
    ...
    if data['community_advprofile_avatar']:
        ...
        if len(avt_name) + directory_name_len > 100:
            ...
            name = name[:96 - len(
                ext) - directory_name_len]  # 96 because django will append _x if filename already exists, and one for dot
            fname = '%s.%s' % (name, ext)
